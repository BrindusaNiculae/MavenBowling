        ...
        while (pins.hasPins(filename, scanner)) {
            ...
            if (roll2.getValue() >= 0) {
                if (roll1.getValue() + roll2.getValue() < 10) {
                    ...
                    readR2 = true;
                    readNR = true;
                } /*SPARE*/ else if (roll1.getValue() + roll2.getValue() == 10) {
                    frame = new Strike();
                    ...
                    frame.setExtraValue1(nextRoll1.getValue());
                    if (pins.hasPins(filename, scanner)) {
                        nextRoll2.setValue(pins.knockDown(filename, scanner));
                    }
                    frame.setExtraValue2(nextRoll2.getValue());
                    frame.calculate(roll1.getValue(), roll2.getValue());
                    
                    roll1.setValue(roll2.getValue());
                    roll2.setValue(nextRoll1.getValue());
                    nextRoll1.setValue(nextRoll2.getValue());
                    readNR = false;
                    readR1 = false;
                    readR2 = false;
                }
